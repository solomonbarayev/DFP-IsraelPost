...
const DashboardStatisticsCards = ({ stats }: DashboardStatisticsCardsProps) => {
    ...
    const activeForms = stats.accountForms?.length || 0;
    ...
    const submittedFormsToday = stats.accountFormSubmissions?.filter(submission => {
        const submissionDate = submission.submittedAt.split(' ')[0];
        return submissionDate === todayString;
    })?.length || 0;

    // Get total submissions
    const totalSubmissions = stats.accountFormSubmissions?.length || 0;

    // Get submissions by status
    const pendingSubmissions = stats.accountFormSubmissions?.filter(
        submission => submission.status === 'pending'
    )?.length || 0;

    // Calculate most active form
    const formSubmissionCounts = stats.accountFormSubmissions?.reduce((acc, submission) => {
        acc[submission.formId] = (acc[submission.formId] || 0) + 1;
        return acc;
    }, {}) || {};

    // Find the form with the most submissions
    const [mostActiveFormId, mostActiveFormCount] = Object.entries(formSubmissionCounts)
        .reduce((max, [formId, count]) => count > max[1] ? [formId, count] : max, ['', 0]);

    const mostActiveFormName = stats.accountForms?.find(
